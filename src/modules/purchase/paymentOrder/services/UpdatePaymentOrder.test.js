...
describe('Purchase - UpdatePaymentOrder', () => {
  describe('when form updated', () => {
    ...
    it('add correct new payment order record', async () => {
      const paymentOrder = await new UpdatePaymentOrder(tenantDatabase, (id = 1), {
        invoiceId: 'PI0050821',
    ...
    it('throw error when require date but it doesnt provide in input', async () => {
      await expect(async () => {
        await new UpdatePaymentOrder(tenantDatabase, (id = 1), {
          supplierId: 1,
          paymentMethodId: 1,
        }).call();
